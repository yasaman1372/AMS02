...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 11 16:11:10 2022
...
from Cuts.ElectronIdentificationCuts import CutTrackerTrackEcalCogDeltaY
...
with uproot.open(address + 'TrackerEcalMatchingYUpperCut.root') as file:
...
Histogram_branches = ['TrackerTrackEcalCogDeltaY', 'EcalEnergyElectronNewMaximumShower', 'McEventWeightElectron']
...
def handle_file(arg):
    ...
    var2_name= 'TrackerTrackEcalCogDeltaY'
    var2_min = -10.0
    var2_max = 10.0
    bin2_num = 200
...
def main():
    ...
    plot.set_ylabel("TrackerTrackEcalCogDeltaY", fontsize = 26)
    plot.set_xscale("log")
   ...
   # cut_events = np.ma.masked_where(cut_events == 0.0, cut_events)	    
    ...
    figure.savefig("TrackerTrackEcalCogDeltaY_cut_new.pdf" , dpi=250)
    plt.close(figure)

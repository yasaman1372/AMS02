...
Branches = Preselection_Branches + Selection_Cuts_Branches + Hist_branches + Identification_Cuts_Branches
...
def main():
   ...
   # plot.set_xlim(0.5,max((var1_binning[1:]+var1_binning[:-1])/2))
    ...
    plt.scatter((var1_binning[1:]+var1_binning[:-1])/2, Ecal_BDT_cut((var1_binning[1:]+var1_binning[:-1])/2), c='r')
   ...
   # plot.errorbar((var2_binning[1:] + var2_binning[:-1]) / 2, Events[-1,:], np.sqrt(Events[-1,:]), fmt=".", label="last bin")
    # or plot as bars
    for i in range(1, 3):
        events= Events[-i,:]/np.sum(Events[-i,:])
       # events = Events[-i,:]
        plot.step(np.concatenate(([var2_binning[0]], var2_binning)), np.concatenate(([0], events, [0])), where="post", label="Energy = "+str(x[-i])+"Gev")
        plot.vlines(Ecal_BDT_cut(x[-i]), min(events),2, label="Ecal cut for Energy = "+str(x[-i])+"Gev", color ='k' )

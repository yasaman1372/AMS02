...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 14 15:24:40 2023
...
Hist_branches = ["TrdPLikelihoodHybridHitsElectron","TotalEnergy3D",'EcalBDT3D','EcalBDTSmoothed_v7_EnergyD',"TrackerTrackGBLMaxSpanRigidity","EcalEnergyElectronNewMaximumShower", "TrdPLikelihoodHybridHitsElectronECAL", 
...
def handle_file(arg):
    ...
    Events = np.zeros((bin2_num, bin1_num))
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
        ...
        if data_type == "MC":
            ...
            hist_values, edges = np.histogramdd((v2,v1), bins= (var2_binning, var1_binning), weights=ak.to_numpy(events.McEventWeightElectron))
       # Events += hist_values

        elif data_type == "ISS":
            hist_values, edges = np.histogramdd((v2,v1), bins= (var2_binning, var1_binning))       
...
def main():
    ...
    Ecal_binn = (var1_binning[1:]+var1_binning[:-1])/2
    Energy_binn= (var2_binning[1:]+var2_binning[:-1])/2
        
    mesh = plot.pcolormesh(Energy_binn, Ecal_binn, events, norm=colors.LogNorm(), cmap =plt.cm.get_cmap('jet'))
    ...
    figure.savefig("2D_Energy_Ecal_Estimator_"+args.data_type+".pdf" , dpi=250)

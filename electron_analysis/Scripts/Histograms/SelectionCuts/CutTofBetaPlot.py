...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  7 13:52:19 2023
...
def main():
    ...
    ax1.set_xlim(0,2.2)
    ax1.set_ylim(10**-4,0.5)
    for axis in ['top','bottom','left','right']:
        ax1.spines[axis].set_linewidth(2)
        
    ax1.set_xlabel("TOF"+" "+r'$\beta$',fontsize = 26,fontweight = 'bold')
    ...
    #eff_pass6 = np.sum(Passed_Events_pass6)/Events_pass6
    ...
    #eff_pass8 =np.sum(Passed_Events_pass6)/Events_pass8
    ...
    ax1.legend(title=''.join(info),title_fontsize=12,fontsize=12,frameon=False, loc="upper left")
    
    ax1.axvline(x=0.8,linestyle=':',color='k',linewidth=5)
    ax1.axvline(x= 1.24,linestyle=':',color='k',linewidth=5)
    
  #  plt.legend()
    
    

    figure.savefig("CutTofBetaPlot.pdf" , dpi=250)
    plt.close(figure)

...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 12 21:34:27 2022
...
Histogram_branches = ['TrackerCharge',"TotalEnergy3D","TrackerTrackGBLMaxSpanChiSquareY","TrackerTrackChoutkoMaxSpanChiSquareY", "EcalEnergyElectronNewMaximumShower", "McEventWeightElectron"]

Branches = Preselection_Branches + Histogram_branches + Identification_Branches + Selection_Branches
...
def handle_file(arg):
    ...
    #var2_name= 'TrackerTrackChoutkoMaxSpanChiSquareY'
    var2_name= 'TrackerTrackGBLMaxSpanChiSquareY'
    var2_min = -3
    var2_max = 2
    bin2_num = 300
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks, branches=Branches):
        ...
        passed_events = CutTrackerChiSquareY(events)
...
def main():
    ...
    ax1.set_ylabel(r"$\rm Tracker \ Track \ \chi^2_{y}$",fontsize = 26)
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    ...
    # plt.ylim(bottom=0)
    # plt.xlim(left=0.5)
    ax1.axhline(y = 0.01 ,linestyle=':',color='k',linewidth=5) 
    ax1.axhline(y = 25 ,linestyle=':',color='k',linewidth=5)
   ...
   #  plot.set_ylabel(r"$\rm Tracker \ Track \ \chi^2_{y}$", fontsize = 26)
   #  plot.set_xscale("log")
   #  plot.set_yscale("log")
   ...
   #  scale=1/events.sum(axis=0)
   #  events = scale*events
   # # events = np.ma.masked_where(events<=1e-3, events)
   # # print(np.ndarray.min(events), flush=True)	    
   ...
   #  figure.savefig("TrackerChiSquareY.pdf" , dpi=250)
   #  plt.close(figure)
   ...
   #  figure.savefig("TrackerChiSquareYEfficiency..pdf" , dpi=250)
   #  plt.close(figure) 

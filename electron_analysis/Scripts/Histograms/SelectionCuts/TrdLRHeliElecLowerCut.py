...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 11 22:33:04 2022
...
cutx = cut.members['fX']
cuty = cut.members['fY']
F_cut = interpolate.interp1d(cutx,cuty)
...
Histogram_branches = ["TotalEnergy3D","TrackerCharge",'TrdPLikelihoodHybridHitsHelium','TrdPLikelihoodHybridHitsElectron',"EcalEnergyElectronNewMaximumShower", "McEventWeightElectron"]
...
def handle_file(arg):
    ...
    var2_name= 'TrdLRHeliElec_Rigidity_HybridHits_TrdP'
    var2_min = 0
    var2_max = 10.0
    bin2_num = 1000
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks, branches=Branches):
        ...
        events = CutTrackerCharge(events)
        ...
        events = CutTrackerTrackEcalCogDeltaY(events)

        passed_events = CutTrdNoHelium(events)
        
        #events = events[(events.TrackerCharge > 1.8) & (events.TrackerCharge < 2.5)]
       # events = CutTrdNoHelium(events)
       # for cut in Tag_Cuts:
           # events = cut(events) #Tag cuts are applied
        
        v1 = ak.to_numpy(events[var1_name])
        v2 = TrdLRHeliElec_Rigidity_HybridHits_TrdP(events) 
        
        v1_passed = ak.to_numpy(passed_events[var1_name])
        v2_passed = TrdLRHeliElec_Rigidity_HybridHits_TrdP(passed_events)
        ...
        if data_type == 'MC': 
            ...
            hist_values_passed, edges_passed = np.histogramdd((v1_passed,v2_passed), bins= (var1_binning, var2_binning), weights=ak.to_numpy(passed_events.McEventWeightElectron))
        
        elif data_type == 'ISS':
            hist_values, edges = np.histogramdd((v1,v2), bins= (var1_binning, var2_binning))
            hist_values_passed, edges_passed = np.histogramdd((v1_passed,v2_passed), bins= (var1_binning, var2_binning))
                     
        Events += hist_values
        Passed_Events += hist_values_passed
...
def main():
    ...
    ax1.set_ylabel("TRD helium estimator", fontsize = 26)
    ax1.set_xscale("log")
    ...
    mesh = ax1.pcolormesh(var1_binning,var2_binning ,events, norm=colors.LogNorm(),cmap =plt.cm.get_cmap('jet'))
    ...
    figure.savefig("TrdLRHeliElecLowercut.pdf" , dpi=250)
    plt.close(figure)
   ...
   # figure.savefig("TrdLRHeliElecLowercutEfficiency.pdf" , dpi=250)
   # plt.close(figure)    

...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 12 15:57:32 2022
...
Histogram_branches = ["TotalEnergy3D",'TrdPActiveLayersHybrid', "EcalEnergyElectronNewMaximumShower", "McEventWeightElectron"]
...
def handle_file(arg):
    ...
    var2_name= 'TrdPActiveLayersHybrid'
    var2_min = -0.5
    var2_max = 20.5
    bin2_num = 21
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks, branches=Branches):
        ...
        passed_events = CutTrdActiveLayers(events)
...
def main():
    ...
    ax1.set_ylabel("TRD active layers", fontsize = 26)
    ax1.set_xscale("log")
    ...
    #ax1.set_xticklabels([]) # remove the x axis numbers
    ax1.axhline(y=15 ,linestyle=':',color='k',linewidth=5)
    ...
    figure.savefig("CutTrdActiveLayers.pdf" , dpi=250)
    plt.close(figure)
    ...
    # plot.set_ylabel("TRD active layers", fontsize = 26)
    # plot.set_xscale("log")
    ...
    # plt.xlim(left=0.5)
    # plt.axhline(y=15 ,linestyle=':',color='k',linewidth=5)   
    # figure.savefig("CutTrdActiveLayers.pdf" , dpi=250)
    # plt.close(figure)
    ...
    # figure.savefig("CutTrdActiveLayersEfficiency..pdf" , dpi=250)
    # plt.close(figure) 

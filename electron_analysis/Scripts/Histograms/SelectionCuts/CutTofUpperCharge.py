...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 12 13:48:42 2022
...
Histogram_branches = ['TofUpperCharge', "McEventWeightElectron"]

Branches = Preselection_Branches + Histogram_branches

#def Energy_OverRigidity(events,version):
 #   if version =="pass6":
...
def handle_file(arg):
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
        ...
        #events = CutEnergyOverRigidity(events,version)
        #if version == "pass8":
         #   events = events[events.TrackerTrackGBLMaxSpanRigidity < 0]
        #elif version == "pass6":
         #   events[events.TrackerTrackChoutkoMaxSpanRigidity < 0]
        ...
        passed_events = CutTofUpperCharge(events)
...
def main():
   ...
   # parser.add_argument("--version", default="", help="version of the input data (e.g. pass8)")

...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  3 12:14:59 2023
...
Hist_branches = ["CCBDT","McEventWeightElectron","TotalEnergy3D","EcalBDT3D","ElectronCCMVABDT","Time","EcalBDT_v7_EnergyD","TrackerTrackGBLMaxSpanRigidity","EcalEnergyElectronNewMaximumShower", "TrdPLikelihoodHybridHitsElectronECAL", 
...
def handle_file(arg):
    ...
    cc_eEvents_st = np.zeros((bin1_num, bin2_num))
    cc_eEvents_mt = np.zeros((bin1_num, bin2_num))
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks, branches=Branches):
       ...
       # events = GeomagneticCutoff(events)
        ...
        #charged confused electron template selection
        #single track
        cc_el_events_st = events_st[events_st.TrackerTrackGBLMaxSpanRigidity > 0]
        cc_el_events_st = CutEcalChiSquareLateralNormalized(cc_el_events_st, "MC")
        #cc_el_events_st = cc_el_events_st[cc_el_events_st.EcalBDT_v7_EnergyD > 0 ]
        cc_el_events_st = cc_el_events_st[cc_el_events_st.EcalBDT3D > - 0.75 ]
        cc_el_events_st = cc_el_events_st[TrdLRElecProt_Energy_HybridHits_TrdP(cc_el_events_st) < 0.75]
        
        
        
               
        ccev1_st = ak.to_numpy(cc_el_events_st[var1_name])
        ccev2_st = ak.to_numpy(cc_el_events_st[var2_name])
        


        cc_ehist_values_st, cc_eedges_st = np.histogramdd((ccev1_st,ccev2_st), bins= (var1_binning, var2_binning),weights=ak.to_numpy(cc_el_events_st.McEventWeightElectron))        
        cc_eEvents_st += cc_ehist_values_st
        
        #multi track 
        cc_el_events_mt = events_mt[events_mt.TrackerTrackGBLMaxSpanRigidity > 0]
        cc_el_events_mt = CutEcalChiSquareLateralNormalized(cc_el_events_mt, "MC")
        #cc_el_events_mt = cc_el_events_mt[cc_el_events_mt.EcalBDT_v7_EnergyD > 0 ]
        cc_el_events_mt = cc_el_events_mt[cc_el_events_mt.EcalBDT3D > -0.75 ]
        cc_el_events_mt = cc_el_events_mt[TrdLRElecProt_Energy_HybridHits_TrdP(cc_el_events_mt) < 0.75]
        
        
        
               
        ccev1_mt = ak.to_numpy(cc_el_events_mt[var1_name])
        ccev2_mt = ak.to_numpy(cc_el_events_mt[var2_name])
        


        cc_ehist_values_mt, cc_eedges_mt = np.histogramdd((ccev1_mt,ccev2_mt), bins= (var1_binning, var2_binning),weights=ak.to_numpy(cc_el_events_mt.McEventWeightElectron))        
        cc_eEvents_mt += cc_ehist_values_mt
    ...
    np.savez(os.path.join(resultdir, f"results_{rank}.npz"), var1_binning=var1_binning,
             var2_binning=var2_binning, cc_eEvents_st=cc_eEvents_st, cc_eEvents_mt=cc_eEvents_mt)
...
def main():
    ...
    for rank in range(args.nprocesses):
        ...
        with np.load(filename) as result_file:
            ...
            if cc_eEvents_st is None:
                cc_eEvents_st = result_file["cc_eEvents_st"]
                cc_eEvents_mt = result_file["cc_eEvents_mt"]
             
            else:
                cc_eEvents_st += result_file["cc_eEvents_st"]
                cc_eEvents_st += result_file["cc_eEvents_mt"]
 

    # now save merged result
    np.savez(os.path.join(args.resultdir, "CCMVABDT_template_MC.npz"), var1_binning=var1_binning, var2_binning=var2_binning,  cc_eEvents_st=cc_eEvents_st, cc_eEvents_mt=cc_eEvents_mt)

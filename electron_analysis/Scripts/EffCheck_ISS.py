...
def handle_file(arg):
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
        ...
        hist_values, hist_edges = np.histogramdd((v3,v2), bins= (var3_binning,var2_binning))      
        Events += hist_values
...
def main():
    ...
    np.savez(os.path.join(args.resultdir, "Efficiency_corrections_uppertof_ISS.npz"),TOF_charge_binning = TOF_charge_binning, TRD_Estimator_binning = TRD_Estimator_binning, Energy_binning = Energy_binning, TCEvents = TCEvents, TCEvents_passed = TCEvents_passed, Events = Events)   

...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug  2 14:38:01 2023
...
def handle_file(arg):
    ...
    for events in read_tree(filename, treename, chunk_size=chunk_size, rank=rank, nranks=nranks):
        ...
        TCevents = NegativeRigidityTag(events)
        ...
        v3=ak.to_numpy(events[var3_name])
        ...
        hist_values, hist_edges = np.histogram(ak.to_numpy(TCevents[var3_name]), bins=var3_binning,weights=ak.to_numpy(TCevents.McEventWeightElectron))       
        Events += hist_values
        
        # this process is done, save result
    np.savez(os.path.join(resultdir, f"results_{rank}.npz"), TRD_Estimator_binning=var1_binning, Energy_binning = var2_binning, TCEvents = TCEvents, TCEvents_passed = TCEvents_passed, Events = Events)
...
def main():
    ...
    np.savez(os.path.join(args.resultdir, "Efficiency_corrections_uppertof_"+args.data_type+".npz"), TRD_Estimator_binning = TRD_Estimator_binning, Energy_binning = Energy_binning, TCEvents = TCEvents, TCEvents_passed = TCEvents_passed, Events = Events)   
